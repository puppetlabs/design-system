...
class Annotations {
  ...
  getLocation(annotation) {
    ...
    if (annotation.highestPoint) {
      ...
      if (point) {
        if (layout === VIZ_TYPES.COMBINATION) {
          layout =
            this.options[point.seriesType] &&
            this.options[point.seriesType].layout;
        }

        if (layout === 'grouped') {
          location.x =
            this.x1(point.seriesLabel) +
            this.x(point.x) +
            this.x1.bandwidth() / 2;
        } else {
  ...
  getTransform(d, padding = 0) {
    ...
    switch (orientation) {
      case 'left':
      case 'right':
        translate = `translate(${point.y + 20 + padding}, ${
          point.x
        }), rotate(90)`;
        break;
      default:
  ...
  render(selection) {
    ...
    const points = this.selection
      .selectAll(CSS.getClassSelector(selector))
      .data(annotations);
    ...
    const point = points
      ...
      .attr(
        'd',
        'M0,2v14l5-5h9c1.105,0,2-0.895,2-2V2c0-1.105-0.895-2-2-2H2C0.895,0,0,0.895,0,2z',
      );
    ...
    if (dispatchers.enabled('annotationClick.external')) {
      point.style('cursor', 'pointer').on('click', annotation => {
        const p = helpers.getMaximumPoint(data, layout);

        dispatchers.call('annotationClick', this, {
          event,
          data: { annotation, point: p },
        });
      });
    }
  }
