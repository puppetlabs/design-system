# Contributing to Insights Charts
...
Insights Charts includes a styleguide that allows us to test components in
isolation.

**To start the styleguide for local development:**
1. Run `npm start`.
2. Open the [styleguide](http://localhost:8080) at http://localhost:8080.
...
3. For any project in which Insights Charts is a dependency, you can link 
  Insights Charts to see your changes reflected in the parent. In the parent
  project, run `npm link @puppet/insights-charts`. Make sure you're using 
  the same version of Node.js in both projects.
...
New versions of Insights Charts are built and published to Artifactory using
